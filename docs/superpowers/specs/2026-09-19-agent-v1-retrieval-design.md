...
检索不到片段时，API 不构造 Prompt，也不调用模型，而是直接返回固定的知识边界回答：`已整理的资料中没有关于这个问题的信息，暂时无法确定。` API 仍返回 `200`，`sources` 返回空数组，因为这是正常的知识边界，不是服务器错误。这样可以避免模型忽略边界提示自行补全，也避免为无关问题消耗模型额度。
...
- 相关问题返回实际命中的来源。
- 无匹配问题由后端确定性短路，返回 `sources: []` 和固定知识边界回答，不调用模型。
