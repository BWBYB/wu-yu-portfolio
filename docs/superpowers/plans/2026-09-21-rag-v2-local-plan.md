...
- [ ] **Step 1: Write failing orchestration tests.** Verify vector results return `mode="vector"`; an embedding/store exception returns lexical chunks with `mode="lexical"` and `fallback_used=True`; blocked requests return no chunks without vector calls; hybrid vector no-match falls back to lexical while strict vector mode can remain no-fallback.
